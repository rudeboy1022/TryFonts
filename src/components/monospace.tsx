...
export const MonoSpace: FC = () => {
  ...
  const [isMonospaceTextToggle, setIsMonospaceTextToggle] =
    useState<boolean>(false);

  const [isMonospaceBackgroundToggle, setIsMonospaceBackgroundToggle] =
    useState<boolean>(false);
  ...
  return (
    <div id="monospaceContainer" css={monospaceContainerStyle}>
      <h2>MonoSpace</h2>
      <div
        className="monospaceBackGroundContainer"
        css={monospaceBackGroundContainerStyle}
        ref={divRef}
      >
        <MonoSpaceTexts
          fontSize={inputFontSizeValue}
          fontWeight={inputFontWeightValue}
          toggle={isMonospaceTextToggle}
      ...
      <div id="buttonContainer" css={buttonContainer}>
        <button
          type="button"
          onClick={handleMonospaceTextToggle}
          css={adjustButtonsStyle}
        >
          {isMonospaceTextToggle
            ? "カラーピッカーを閉じる"
            : "文字色のカラーピッカー"}
        </button>
        <button
          type="button"
          onClick={handleMonospaceBackgroundToggle}
          css={adjustButtonsStyle}
        >
          {isMonospaceBackgroundToggle
            ? "カラーピッカーを閉じる"
